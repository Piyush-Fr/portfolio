...
export default function Practice() {
  const [mobileModalSkill, setMobileModalSkill] = useState<SkillItem | null>(null);

  const handleSkillClick = (skill: SkillItem) => {
    if (typeof window !== 'undefined' && window.innerWidth < 768) {
      setMobileModalSkill(skill);
    }
  };
  ...
  return (
    <>
              ...
              {leftSkills.map((item) => (
                <RevealText key={item.num} delay={Number(item.num) * 0.08}>
                  <div 
                    ...
                    className="py-5 border-b border-grid-line flex items-center justify-between gap-4 cursor-pointer group hover:border-brand-red transition-colors"
                  >
      ...
      {/* Strict Mobile-Only Modal for Skill Details */}
      {mobileModalSkill && (
        <div 
          ...
          onClick={() => setMobileModalSkill(null)}
        >
          <div 
            ...
            <div className="flex justify-between items-start mb-4 border-b border-grid-line pb-4">
              <div>
                <span className="font-mono text-xs text-brand-red uppercase tracking-widest block mb-1">
                  SKILL {mobileModalSkill.num} · {mobileModalSkill.category}
                </span>
                <h3 className="text-2xl sm:text-3xl font-bold tracking-tight text-white">
                  {mobileModalSkill.title}
                </h3>
              </div>
              <button
                onClick={() => setMobileModalSkill(null)}
